...
const parsePositiveInt = (value?: string, defaultValue?: number): number | undefined => {
    ...
    if (!Number.isInteger(n) || n <= 0) throw new BadRequestError(`Параметр должен быть положительным числом`);
    return n;
};
...
const parseNonNegativeNumber = (value?: string): number | undefined => {
    ...
    const n = Number(value);
    if (!Number.isFinite(n) || n < 0) throw new BadRequestError(`Параметр должен быть неотрицательным числом`);
    return n;
};
...
const parseDate = (value?: string): Date | undefined => {
    ...
    if (Number.isNaN(d.getTime())) throw new BadRequestError(`Некорректная дата: ${value}`);
    return d;
};

const parseSortOrder = (value?: string, def: "asc" | "desc" = "desc"): "asc" | "desc" => {
    if (!value) return def;
    if (value !== "asc" && value !== "desc") throw new BadRequestError('sortOrder должен быть "asc" или "desc"');
    return value;
};

export const sanitizeSearch = (value?: string): string | undefined => {
    if (!value) return undefined;
